...
# ==============================================================================
st.set_page_config(
    page_title="AREKO BCA - Let’s Make Recap Less Recap-y",
    page_icon="💳",
    layout="wide",
...
with st.sidebar:
    ...
    cabang = st.text_input("Cabang", key=f"cabang_{ver}", placeholder="Contoh: KCU BCA Thamrin Jakarta")
    nama_cust = st.text_input("Nama Cust", key=f"cust_{ver}", placeholder="Nama lengkap customer")
    no_rek = st.text_input("Nomor Rekening", key=f"rek_{ver}", placeholder="Contoh: 1234567890")

    nama_bank = st.text_input("Nama Bank", value="BCA", disabled=True, key=f"bank_{ver}")
...
<div style="margin-bottom: 24px;">
    <h1 style="font-size: 2.2rem; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -0.02em;">AREKO - BCA</h1>
    <p style="font-size: 1.05rem; font-weight: 500; color: #64748b; margin-top: 4px; margin-bottom: 0;">Let’s Make Recap Less Recap-y • Bank Central Asia</p>
</div>
""", unsafe_allow_html=True)

st.markdown("#### 📂 Berkas Rekening Koran BCA")
uploaded_files = st.file_uploader(
    "Pilih 1 hingga 3 Berkas Rekening Koran Bank BCA (Format PDF)",
    type=["pdf"],
    accept_multiple_files=True,
    key=f"uploader_{ver}",
    help="Unggah dokumen PDF mutasi rekening BCA untuk dianalisis dan direkapitulasi secara otomatis."
)
...
if uploaded_files:
    ...
    if proses_clicked:
        ...
        for i, file_obj in enumerate(uploaded_files):
            ...
            extracted = parse_bca(file_obj, filename=file_obj.name)
            resume_list.append(extracted)

        st.session_state["resume_list"] = resume_list
        progress_bar.empty()
        st.toast("Ekstraksi data mutasi BCA berhasil selesai!", icon="✅")
        st.rerun()
...
if st.session_state.get("resume_list"):
    ...
    st.markdown("#### 📋 Rekapitulasi Mutasi Rekening BCA")
    st.dataframe(styled_df, use_container_width=True, hide_index=True)

    # 2. Section Unduh Hasil
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("#### 📥 Unduh Dokumen Form Validasi BCA")
    ...
    with col_dl1:
        st.markdown(
            """
            <div class="dl-card">
                <div class="dl-card-title">📄 Dokumen PDF Resmi BCA</div>
        ...
        st.download_button(
            label="Unduh PDF Form Validasi BCA",
            data=pdf_bytes,
            file_name="FORM_VALIDASI_MUTASI_REKENING_BCA.pdf",
    ...
    with col_dl2:
        st.markdown(
            """
            <div class="dl-card">
                <div class="dl-card-title">📊 Dokumen Excel Spreadsheet BCA</div>
        ...
        st.download_button(
            label="Unduh Excel Form Validasi BCA (.xlsx)",
            data=xlsx_bytes,
            file_name="FORM_VALIDASI_MUTASI_REKENING_BCA.xlsx",
    ...
    <div class="empty-state-box" style="text-align: center; display: flex; flex-direction: column; align-items: center; justify-content: center;">
        ...
        <div class="empty-state-title" style="text-align: center;">Belum Ada Rekening Koran BCA yang Diproses</div>
        <p class="empty-state-desc" style="text-align: center; margin: 0 auto;">
            Silakan unggah berkas PDF rekening koran BCA di atas, lengkapi parameter dokumen di sidebar, lalu klik <b>Ekstraksi & Rekap Data</b> untuk menampilkan tabel ringkasan mutasi dan mengunduh formulir validasi.
        </p>
    </div>
    """, unsafe_allow_html=True)
